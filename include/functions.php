...
if (!function_exists("getNodesByFontString")) {
    ...
    function getNodesByFontString($font_id = '')
    {
        if (!file_exists($cache = getCacheFilename(FONTS_CACHE , '%snodes-by-font--%s.json',sha1($font_id), 'nodes')))
        {
            ...
            try
            {
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` WHERE `font_id` = '$font_id'";
                $result = $GLOBALS['APIDB']->queryF($sql);
                while($row = $GLOBALS['APIDB']->fetchArray($result))
                {
                    $nodesid[$row['node_id']] = $row['node_id'];
                }
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes') . "` WHERE `id` IN ('".implode("','", $nodesid) ."') AND `type` = 'keys' ORDER BY `node` DESC";
                $nodocity = $GLOBALS['APIDB']->queryF($sql);
                while($node = $GLOBALS['APIDB']->fetchArray($nodocity))
                {
...
if (!function_exists("getPeerIdentity")) {
    ...
    function getPeerIdentity( $uri, $callback, $zip, $fonts, $version, $polinating = true, $root = "http://fonts.labs.coop" ) {
        ...
        } elseif ($GLOBALS['APIDB']->getRowsNum($results = $GLOBALS['APIDB']->queryF(sprintf($sql, $GLOBALS['APIDB']->escape($uri))))==1)
        {
            ...
            if (strpos($uri, 'localhost')>0||strpos($uri, 'labs.coop')>0)
                $polinating = false;
                $sql = "INSERT INTO `" . $GLOBALS['APIDB']->prefix('peers') . "` (`peer-id`, `polinating`, `api-uri`, `api-uri-callback`, `api-uri-zip`, `api-uri-fonts`, `version`, `created`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')";
                $peerid = md5($uri.$version.$callback.$zip.$fonts.$polinating.$root.microtime(true));
                if ($GLOBALS['APIDB']->queryF(sprintf($sql, $GLOBALS['APIDB']->escape($peerid), ($polinating==true?"yes":"no"), $GLOBALS['APIDB']->escape($uri), $GLOBALS['APIDB']->escape($callback), $GLOBALS['APIDB']->escape($zip), $GLOBALS['APIDB']->escape($fonts), $GLOBALS['APIDB']->escape($version), time())) && $polinating == true)
                {
...
if (!function_exists("getFontsReleased")) {
    ...
    {
        list($total) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT count(*) from `" . $GLOBALS['APIDB']->prefix('fonts') . "` `a` INNER JOIN `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` `b` ON `a`.`id` =  `b`.`font_id` WHERE `a`.`state` = 'online'"));
        return $total;
    }
}
...
if (!function_exists("getFontsWaitingConvertions")) {
    ...
    {
        list($total) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT count(*) from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `uploaded` > '0' AND `converted` = '0' AND `storaged` = '0'"));
        return $total;
    }
}
...
if (!function_exists("getFontsWaitingConverted")) {
    ...
    {
        list($total) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT count(*) from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `uploaded` > '0' AND `converted` > '0' AND `quizing` > '0' AND `storaged` <= '0'  AND (`finished` >= `needing` OR `expired` < UNIX_TIMESTAMP())"));
        return $total;
    }
}
...
if (!function_exists("getSurveysWaiting")) {
    ...
    {
        list($total) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT count(*) from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `converted` > 0 AND `quizing` = 0 AND `expired` = 0"));
        return $total;
    }
}
...
if (!function_exists("getSurveysQueued")) {
    ...
    {
        list($total) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT count(*) from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `converted` > 0 AND `quizing` > 0 AND `expired` < unix_timestamp() AND `storaged` = 0"));
        return $total;
    }
}
...
if (!function_exists("getSurveysExpiring")) {
    ...
    {
        list($total) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT count(*) from `" . $GLOBALS['APIDB']->prefix('flows_history') . "` WHERE `expiring` <= '$start' AND `expiring` >= '$end' AND `step` = 'waiting'"));
        return $total;
    }
}
...
if (!function_exists("getFileDIZ")) {
    ...
    function getFileDIZ($font_id = 0, $upload_id = 0, $fingerprint = '', $filename = '', $bytes = 0, $filez = array()) {
        ...
        {
            $upload = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `font_id` = '".$font_id."'"));
            $font = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE `id` = '".$font_id."'"));
        } else {
            $upload = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `id` = '".$upload_id."'"));
            if (isset($upload['font_id']))
                $font = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE `id` = '".$upload['font_id']."'"));
        }
        ...
        if (!empty($datastore['name']))
            ...
            if (!empty($datastore['bizo']))
                ...
                $result = $GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_contributors') . "` WHERE `upload_id` = '".$upload['id']."'");
                while($contributor = $GLOBALS['APIDB']->fetchArray($result)){
                    $net = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('networking') . "` WHERE `ip_id` = '".$contributor['ip_id']."'"));
                ...
                $result = $GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_downloads') . "` WHERE `font_id` = '".$font['id']."' ORDER BY `when` DESC");
                while($download = $GLOBALS['APIDB']->fetchArray($result)){
                    $net = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('networking') . "` WHERE `ip_id` = '".$download['ip_id']."'"));
...
if (!function_exists("getReserves")) {
    ...
    function getReserves($fontname = '') {
        ...
        $result = $GLOBALS["APIDB"]->queryF("SELECT * FROM `reserves` ORDER BY `parent` DESC, `child` ASC");
        while($row = $GLOBALS["APIDB"]->fetchArray($result))
        {
    ...
    function getPreviewHTML($mode = '', $clause = '', $state = '', $name = '', $output = '', $version = "v2")
    {
        ...
        switch($mode)
        {
                    ...
                    case "png":
                        ...
                        if (!file_exists($font = getCacheFilename(FONT_RESOURCES_CACHE, "%sfont--%s.ttf", md5($clause), "ttf")))
                        {
                            ...
                            if (!is_dir(FONTS_CACHE))
                                ...
                                $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` SET `cachings` = `cachings` + 1, `cached` = UNIX_TIMESTAMP() WHERE `type` = 'ttf AND `font_id` = '$clause'");
                        }
            ...
            case "fonts":
                ...
                {
                    $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts') . "` SET `hits` = `hits` + 1 WHERE `id` = '" . $key . "'");
                    $fonts = array();
...
if (!function_exists("getFontIdentitiesArray")) {
    ...
    {
        $ret=array();
        $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "`";
        $result = $GLOBALS['APIDB']->queryF($sql);
...
if (!function_exists("getRegionalFontName")) {
    ...
    function getRegionalFontName($fontid = '', $latitude = 0, $longitude = 0, $getGistance = false, $json = array())
    {
        ...
        if (!isset($variables[$fontid]))
        {
            ...
            list($name, $distance) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT `name`, 3956 * 2 * ASIN(SQRT(POWER(SIN((" . abs($latitude) . " - abs(`latitude`)) * pi() / 180 / 2), 2) + COS(" . abs($latitude) . " * pi() / 180 ) * COS(abs(`latitude`) *  pi() / 180) * POWER(SIN((" . $longitude . " - `longitude`) *  pi() / 180 / 2), 2) )) as distance FROM `" . $GLOBALS['APIDB']->prefix('fonts_names') . "` WHERE `font_id` = '$fontid' ORDER BY `distance` LIMIT 1"));
            if (empty($json)&&empty($name))
            {
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` WHERE (`font_id` = '$fontid')";
                if (!$result = $GLOBALS['APIDB']->queryF($sql))
                    die("SQL Failed: $sql;");
                    while($row = $GLOBALS['APIDB']->fetchArray($result))
                    {
                        $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts') . "` SET `hits` = `hits` + 1 WHERE `id` = '" . $row['font_id'] . "'");
                        $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` SET `accessings` = `accessings` + 1, `accessed` = UNIX_TIMESTAMP() WHERE `id` = '" . $row['id'] . "'");
                        ...
                        switch($font['medium'])
                        {
                            ...
                            case 'FONT_RESOURCES_RESOURCE':
                                if ($font['medium'] == 'FONT_RESOURCES_CACHE')
                                {
                                    ...
                                    if (!file_exists(constant($font['medium']) . $row['path'] . DIRECTORY_SEPARATOR . $row['filename']) && !isset($sessions[md5($font['path'] . DIRECTORY_SEPARATOR . $font['filename'])]))
                                    {
                                        ...
                                        $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` SET `cachings` = `cachings` + 1, `cached` = UNIX_TIMESTAMP() WHERE `id` = '" . $row['id'] . "'");
                                    } else {
                            ...
                            case 'FONT_RESOURCES_PEER':
                                ...
                                if (!file_exists(constant(FONT_RESOURCES_CACHE) . $row['path'] . DIRECTORY_SEPARATOR . $row['filename']) && !isset($sessions[md5($font['path'] . DIRECTORY_SEPARATOR . $font['filename'])]))
                                {
                                    ...
                                    if ($GLOBALS['APIDB']->getRowsNum($results = $GLOBALS['APIDB']->queryF(sprintf($sql, $GLOBALS['APIDB']->escape($font['peer_id']))))==1)
                                    {
                                        $peer = $GLOBALS['APIDB']->fetchArray($results);
                                        mkdir(constant("FONT_RESOURCES_CACHE") . $row['path'], 0777, true);
...
if (!function_exists("getCSSListArray")) {
    ...
    function getCSSListArray($mode = '', $clause = '', $state = '', $name = '', $output = '', $version = "v2")
    {
        ...
        if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, "%s$mode-css-list-clause--%s.json", sha1($mode.$clause.$output.$state.$name.$version), "css")))
        {
            ...
            {
                case "font":
                    $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE `id` = '$clause'";
                    $result = $GLOBALS['APIDB']->queryF($sql);
                    while($font = $GLOBALS['APIDB']->fetchArray($result))
                    {
                        foreach(getArchivingShellExec() as $type => $exec)
                            ...
                            $fonts = array();
                            $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts') . "` SET `hits` = `hits` + 1 WHERE `id` = '" . $clause . "'");
                            foreach(array_keys(fontsUseragentSupportedArray()) as $fonttype)
                            {
                ...
                case "fonts":
                    ...
                    foreach(getFontsListArray($clause, $output) as $key => $font)
                    {
                        ...
                        foreach(getArchivingShellExec() as $type => $exec)
                            ...
                            $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_names') . "` WHERE `font_id` = '$key'";
                            $result = $GLOBALS['APIDB']->queryF($sql);
                            while($fontname = $GLOBALS['APIDB']->fetchArray($result))
                            {
            ...
            foreach($_GET as $key => $value)
                if (empty($value) && is_numeric($key))
                    ...
                    if ($items < 5)
                        ...
                        {
                            case "zeroday":
                                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE (`released` > 0) ORDER BY `released` DESC LIMIT $items";
                                $result = $GLOBALS['APIDB']->queryF($sql);
                                while($row = $GLOBALS['APIDB']->fetchArray($result))
                                {
                                    $xml .= "\n\n\t<item>";
                            ...
                            default:
                            case "popular":
                                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE (`hits` > 0) ORDER BY `hits` DESC LIMIT $items";
                                $result = $GLOBALS['APIDB']->queryF($sql);
                                while($row = $GLOBALS['APIDB']->fetchArray($result))
                                {
                                    $xml .= "\n\n\t<item>";
...
if (!function_exists("getSurveyCSSListArray")) {
    ...
    {
        $row = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF("SELECT * FROM `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `key` = '$clause'"));
        if (count($row)>0)
        {
            ...
            if (!$GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('networking') . "` SET `" . $GLOBALS['APIDB']->prefix('fonts') . "` = `" . $GLOBALS['APIDB']->prefix('fonts') . "` + 1 WHERE `ip_id` LIKE '$ipid'"))
                ...
                if (!$result = $GLOBALS['APIDB']->queryF($sql))
                    ...
                    while($row = $GLOBALS['APIDB']->fetchArray($result))
                    {
                        ...
                        $fontfiles = $GLOBALS['APIDB']->fetchArray($GLOBALS['APIDB']->queryF($sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` WHERE `font_id` = '" . $row['font_id'] . "' AND `type` = '$output'"));
                        $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts') . "` SET `hits` = `hits` + 1, `accessed` = UNIX_TIMESTAMP() WHERE `id` = '" . $row['font_id'] . "'");
                        $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` SET `hits` = `hits` + 1, `accessed` = UNIX_TIMESTAMP() WHERE `id` = '" . $row['id'] . "'");
                        $resultb = $GLOBALS['APIDB']->queryF($sql = "SELECT * FROM `" . $GLOBALS['APIDB']->prefix('fonts_callbacks') . "` WHERE `failed` <= unix_timestamp() - (3600 * 6) AND LENGTH(`uri`) > 0 AND `type` IN ('fonthit') AND `font_id` = '" . $row['font_id'] . "'");
                        while($callback = $GLOBALS['APIDB']->fetchArray($resultb))
                        {
                            @setCallBackURI($callback['uri'], 145, 145, array_merge(array('type' => $output, 'hits' => $fontfiles['hits']+1, 'font-key' => $row['font_id'], 'ipid' => getIPIdentity('', true))), array("success"=>"UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_callbacks') . "` SET `calls` = `calls` + 1, `last` = UNIX_TIMESTAMP() WHERE `id` = '" . $callback['id'] . "'", "failed" => "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_callbacks') . "` SET `calls` = `calls` + 1, `last` = UNIX_TIMESTAMP(), `failed` = UNIX_TIMESTAMP() WHERE `id` = '" . $callback['id'] . "'"));
                        }
                        $resdata = array();
    ...
    function setFontCallback($mode = '', $clause = '', $state = '', $output = '', $version = '')
    {
        ...
        foreach($_REQUEST as $key => $value)
            ...
            if (isset($values['email']) || !empty($values['email'])) {
                ...
                $sql = "Select count(*) from `" . $GLOBALS['APIDB']->prefix('fonts_callbacks') . "` WHERE `type` = '$mode' and `font_id` = '$clause' and `email` = '".$GLOBALS['APIDB']->escape($values['email']) . "' AND `uri` = '".$GLOBALS['APIDB']->escape($values['uri']) . "'";
                ...
                $sql = "Select `id` from `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` WHERE `font_id` = '$clause'";
                list($archiveid) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF($sql));
                $sql = "Select `id` from `" . $GLOBALS['APIDB']->prefix('uploads') . "` WHERE `font_id` = '$clause'";
                list($uploadid) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF($sql));
                $sql = "INSERT INTO `" . $GLOBALS['APIDB']->prefix('fonts_callbacks') . "` (`type`, `email`, `uri`, `font_id`, `archive_id`, `upload_id`) VALUES('$mode', '".$GLOBALS['APIDB']->escape($values['email']) . "','".$GLOBALS['APIDB']->escape($values['uri']) . "', '$clause', '$archiveid', '$uploadid')";
                if (!$GLOBALS['APIDB']->queryF($sql))
                    die("SQL Failed: $sql;");
...
if (!function_exists("getFontsCallbacksArray")) {
    ...
    function getFontsCallbacksArray($clause = '', $state = '', $output = '', $version = '')
    {
        ...
        {
            $return = array();
            $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_callbacks') . "` WHERE (`font_id` = '$clause')";
            $result = $GLOBALS['APIDB']->queryF($sql);
            while($row = $GLOBALS['APIDB']->fetchArray($result))
            {
                unset($row['id']);
...
if (!function_exists("getFontDownload")) {
    ...
    function getFontDownload($mode = '', $clause = '', $state = '', $output = '', $version = '')
    {
        ...
        while($row = $GLOBALS['APIDB']->fetchArray($result))
        {
            ...
            $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts') . "` SET `downloaded` = `downloaded` + 1, `accessed` = UNIX_TIMESTAMP() WHERE `id` = '" . $row['font_id'] . "'");
            $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` SET `sourcings` = `sourcings` + 1, `sourced` = UNIX_TIMESTAMP() WHERE `id` = '" . $row['id'] . "'");
            ...
            if (!mkdir($currently = FONT_RESOURCES_SORTING . DIRECTORY_SEPARATOR .$state . DIRECTORY_SEPARATOR . $ipid. DIRECTORY_SEPARATOR . $row['font_id'], 0777, true))
                if (!is_dir($currently))
                    ...
                    if (!$GLOBALS['APIDB']->queryF($sql = "INSERT INTO `" . $GLOBALS['APIDB']->prefix('fonts_downloads') . "` (`font_id`, `archive_id`, `filename`, `ip_id`, `when`) VALUES ('" . $row['font_id'] . "', '" . $row['id'] . "', '$filename', '$ipid', unix_timestamp())"))
                        die("SQL Failed: $sql;");
                        if (!$GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('networking') . "` SET `downloads` = `downloads` +1 WHERE `ip_id` LIKE '$ipid'"))
                            die("SQL Failed: $sql;");
        ...
        if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%sfont-ufo-by-id--%s.json', sha1($clause.$ufofile), $output)))
        {
            ...
            while($row = $GLOBALS['APIDB']->fetchArray($result))
            {
                ...
                } elseif (substr($state, strlen($state)-1, 1) == "/")
                {
                    ...
                    $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` SET `accessings` = `accessings` + 1, `accessed` = UNIX_TIMESTAMP() WHERE `path` LIKE '" . dirname($state) . "' AND `font_id` = '".$row['font_id']."'");
                } elseif (substr($state, strlen($state)-1, 1) != "/" && strlen($state)) {
                    $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` SET `sourcings` = `sourcings` + 1, `sourced` = UNIX_TIMESTAMP() WHERE `filename` = '" . basename($state) . "' AND `path` LIKE '" . dirname($state) . "' AND `font_id` = '".$row['font_id']."'");
...
if (!function_exists("getFontsByNodeListArray")) {
    ...
    function getFontsByNodeListArray($node_id = 0, $nochain = false)
    {
        if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%snode-identities-by-id--%s.json', sha1($node_id.$nochain), 'array')))
        {
            ...
            $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` WHERE `node_id` = '$node_id'";
            $result = $GLOBALS['APIDB']->queryF($sql);
            while($row = $GLOBALS['APIDB']->fetchArray($result))
            {
            ...
            if (count($fontsid))
            {
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` WHERE `font_id` IN ('".implode("', '", $fontsid) ."') ORDER BY `font_id` ASC";
                $archives = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($archives))
                {
                    $archive[$data['font_id']] = $data;
                    unset($archive[$data['font_id']]['font_id']);
                }
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` WHERE `font_id` IN ('".implode("', '", $fontsid) ."') ORDER BY `font_id` ASC, `path` ASC, `filename` ASC";
                $files = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($files))
                {
                    $file[$data['font_id']][md5($data['id'].$data['font_id'])] = $data;
                    unset($file[$data['font_id']][md5($data['id'].$data['font_id'])]['font_id']);
                    
                }
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_names') . "` WHERE `font_id` IN ('".implode("', '", $fontsid) ."') ORDER BY `font_id` ASC, `name` ASC";
                $names = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($names))
                {
                    $name[$data['font_id']][md5($data['name'].$data['font_id'])] = $data;
                    unset($file[$data['font_id']][md5($data['name'].$data['font_id'])]['font_id']);
                    
                }
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE `font_id` IN ('".implode("', '", $fontsid) ."') ORDER BY `font_id` ASC";
                $fontages = $GLOBALS['APIDB']->queryF($sql);
                while($font = $GLOBALS['APIDB']->fetchArray($fontages))
                {
                    if ($nochain==true)
...
if (!function_exists("getNodesByFontListArray")) {
    ...
    function getNodesByFontListArray($font_id = '', $nochain = false)
    {
        if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%sfont-nodes-by-id--%s.json', sha1($font_id.$nochain), 'array')))
        {
            ...
            while($row = $GLOBALS['APIDB']->fetchArray($result))
            {
                $ncity = $GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes') . "` WHERE `id` = '".$row['node_id']."'");
                while($node = $GLOBALS['APIDB']->fetchArray($ncity))
                {
    ...
    function getFontsByNodesListArray($node_id = '')
    {
        if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%sfont-noding--%s.json', sha1($node_id), 'array')))
        {
            try {
                ...
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` WHERE `node_id` = '$node_id' ORDER BY `font_id` ASC";
                $result = $GLOBALS['APIDB']->queryF($sql);
                while($row = $GLOBALS['APIDB']->fetchArray($result))
                {
                    $font_ids[$row['font_id']] = $row['font_id'];
                }
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` WHERE `font_id` IN ('".implode("', '", $font_ids) ."') ORDER BY `font_id` ASC";
                $archives = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($archives))
                {
                    $archive[$data['font_id']] = $data;
                ...
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` WHERE `font_id` IN ('".implode("', '", $font_ids) ."') ORDER BY `font_id` ASC, `path` ASC, `filename` ASC";
                $files = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($files))
                {
                    $file[$data['font_id']][md5($data['id'].$data['font_id'])] = $data;
                ...
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_names') . "` WHERE `font_id` IN ('".implode("', '", $font_ids) ."') ORDER BY `font_id` ASC, `name` ASC";
                $names = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($names))
                {
                    $name[$data['font_id']][md5($data['name'].$data['font_id'])] = $data;
                ...
                $ncity = $GLOBALS['APIDB']->queryF($sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE `id` IN ('".implode("','", $font_ids)."')");
                while($font = $GLOBALS['APIDB']->fetchArray($ncity))
                {
                    try {
                        $downloads = array();
                        foreach(getArchivingShellExec() as $type => $exec)
        ...
        if (!isset($_SESSION["randoms"]['fontee'][md5($_SERVER["REQUEST_URI"])]))
        {
            if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%sfont-random-nodes--%s.json', sha1($nodestring.$_SERVER["REQUEST_URI"]), 'array')))
            {
                if (substr(strtolower($nodestring),0,3)!='any')
                {
                    ...
                    $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes') . "` WHERE `node` IN ('".str_replace("-", "','", $nodestring). "')";
                    $result = $GLOBALS['APIDB']->queryF($sql);
                    while($row = $GLOBALS['APIDB']->fetchArray($result))
                    {
                        $nodes_id[$row['id']] = $row['id'];
                    }
                    $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` WHERE `node_id` IN ('".implode("','", $nodes_id)."') ORDER BY RAND()";
                    $nodocity = $GLOBALS['APIDB']->queryF($sql);
                    while($font = $GLOBALS['APIDB']->fetchArray($nodocity))
                    {
                        $fonts_id[$font['font_id']] = $font['font_id'];
                    }
                    $sql = "SELECT DISTINCT `a`.* FROM `" . $GLOBALS['APIDB']->prefix('fonts') . "` `a` INNER JOIN `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` `b` ON `a`.`id` =  `b`.`font_id` WHERE `a`.`state` = 'online' AND `a`.`id` IN ('".implode("','", $fonts_id)."') " . (!empty($normal)?" AND `a`.`normal` = $normal":"") . (!empty($normal)?" AND `a`.`bold` = $bold":"") . (!empty($italic)?" AND `a`.`italic` = $italic":"") . (!empty($condensed)?" AND `a`.`condensed` = $condensed":"") . " ORDER BY RAND() LIMIT 1";
                    if (!$fonteo = $GLOBALS['APIDB']->queryF($sql))
                        die("SQL Failed: $sql;");
                        while($fontee = $GLOBALS['APIDB']->fetchArray($fonteo))
                        {
                            @writeRawFile($cache, json_encode($fontee));
                            return $_SESSION["randoms"]['fontee'][md5($_SERVER["REQUEST_URI"])] = $fontee;
                        }
                } else {
                    $sql = "SELECT DISTINCT `a`.* FROM `" . $GLOBALS['APIDB']->prefix('fonts') . "` `a` INNER JOIN `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` `b` ON `a`.`id` =  `b`.`font_id` WHERE `a`.`state` = 'online'". (!empty($normal)?" AND `a`.`normal` = $normal":"") . (!empty($normal)?" AND `a`.`bold` = $bold":"") . (!empty($italic)?" AND `a`.`italic` = $italic":"") . (!empty($condensed)?" AND `a`.`condensed` = $condensed":"") . " ORDER BY RAND() LIMIT 1";
                    if (!$fonteo = $GLOBALS['APIDB']->queryF($sql))
                        die("SQL Failed: $sql;");
                        while($fontee = $GLOBALS['APIDB']->fetchArray($fonteo))
                        {
        ...
        if (!isset($_SESSION["randoms"]['fontid'][md5($toponly.$_SERVER["REQUEST_URI"])]))
        {
            if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%sfont-fingerprint--%s.md5', sha1($nodestring.$toponly.$_SERVER["REQUEST_URI"]), 'hash')))
            {
                if (substr(strtolower($nodestring),0,3)!='any')
                {
                    ...
                    while ($tries<20)
                    {
                        $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` WHERE `node_id` IN ('".implode("','", $node_id)."') ORDER BY RAND() LIMIT 1";
                        $nodocity = $GLOBALS['APIDB']->queryF($sql);
                        while($node = $GLOBALS['APIDB']->fetchArray($nodocity))
                        {
                            $sql = "SELECT DISTINCT count(*) FROM `" . $GLOBALS['APIDB']->prefix('fonts') . "` `a` INNER JOIN `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` `b` ON `a`.`id` =  `b`.`font_id` WHERE `a`.`state` = 'online' AND `a`.`id` LIKE '".$node['font_id']."'";
                            list($count) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF($sql));
                            if ($count>0)
                            {
                    ...
                    while ($tries<20)
                    {
                        $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` ORDER BY RAND() LIMIT 1";
                        $nodocity = $GLOBALS['APIDB']->queryF($sql);
                        while($node = $GLOBALS['APIDB']->fetchArray($nodocity))
                        {
                            $sql = "SELECT DISTINCT count(*) FROM `" . $GLOBALS['APIDB']->prefix('fonts') . "` `a` INNER JOIN `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` `b` ON `a`.`id` =  `b`.`font_id` WHERE `a`.`state` = 'online' AND `a`.`id` LIKE '".$node['font_id']."'";
            ...
            if (!isset($_SESSION["randoms"]['nodeids'][md5($toponly.$_SERVER["REQUEST_URI"])]))
            {
                if (!file_exists($cacheb = getCacheFilename(FONT_RESOURCES_CACHE, '%sfonts-random-nodes--%s.json', sha1($nodestring.$toponly.$_SERVER["REQUEST_URI"]), 'array')))
                {
                    if (substr(strtolower($nodestring),0,3)!='any')
                    {
                        ...
                        $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` WHERE `node_id` IN ('".implode( "','", $node_ids). "')";
                        $nodocity = $GLOBALS['APIDB']->queryF($sql);
                        while($node = $GLOBALS['APIDB']->fetchArray($nodocity))
                        {
                        ...
                        $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes_linking') . "` ORDER BY RAND() LIMIT " . mt_rand(1,27);
                        $nodocity = $GLOBALS['APIDB']->queryF($sql);
                        while($node = $GLOBALS['APIDB']->fetchArray($nodocity))
                        {
                            if (!isset($ids[$node['font_id']]))
...
if (!function_exists("getFontsIDsNodeStringArray")) {
    ...
    function getFontsIDsNodeStringArray()
    {
        ...
        {
            $ncity = $GLOBALS['APIDB']->queryF($sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` ORDER BY `created` ASC");
            while($font = $GLOBALS['APIDB']->fetchArray($ncity))
            {
        ...
        if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%sfonts-listings--%s.json', sha1($clause.$state), 'array')))
        {
            ...
            switch ($clause)
            {
                case "all":
                    if (!empty($local))
                        $local = " WHERE $local";
                        $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "`$local ORDER BY `" . $GLOBALS['APIDB']->prefix('nodes') . "` DESC$limits";
                        break;
                default:
                    if (!empty($local))
                        $local = " AND $local";
                        $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE `id` IN ('".implode("','", @getFontsIDsFromNodesList($clause, false))."')$local ORDER BY `" . $GLOBALS['APIDB']->prefix('nodes') . "` DESC$limits";
                        break;
            }
            $result = $GLOBALS['APIDB']->queryF($sql);
            while($font = $GLOBALS['APIDB']->fetchArray($result))
            ...
            if (count($fontids))
            {
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` WHERE `font_id` IN ('".implode("', '", $fontids) ."') ORDER BY `font_id` ASC";
                $archives = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($archives))
                {
                    $archive[$data['font_id']] = $data;
                ...
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` WHERE `font_id` IN ('".implode("', '", $fontids) ."') ORDER BY `font_id` ASC, `path` ASC, `filename` ASC";
                $files = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($files))
                {
                    $file[$data['font_id']][md5($data['id'].$data['font_id'])] = $data;
                ...
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_names') . "` WHERE `font_id` IN ('".implode("', '", $fontids) ."') ORDER BY `font_id` ASC, `name` ASC";
                $names = $GLOBALS['APIDB']->queryF($sql);
                while($data = $GLOBALS['APIDB']->fetchArray($names))
                {
                    $name[$data['font_id']][md5($data['name'].$data['font_id'])] = $data;
                ...
                $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts') . "` WHERE `id` IN ('".implode("', '", $fontids) ."') ORDER BY `id` ASC";
                $result = $GLOBALS['APIDB']->queryF($sql);
                while($font = $GLOBALS['APIDB']->fetchArray($result))
                {
        ...
        if ($state=='cron')
            ...
            if (file_exists($unlink = FONTS_CACHE . DIRECTORY_SEPARATOR . date("Y-m-W-H", time() - 3600 *24 * 7) . 'nodes-listing--' . sha1($clause.$state).'.json'))
                ...
                if (!file_exists($cache = getCacheFilename(FONT_RESOURCES_CACHE, '%snodes-listings--%s.json', sha1($clause.$state), 'array')))
                {
                    ...
                    {
                        default:
                            $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes') . "` ORDER BY `weight`, `usage` DESC".$limits;
                            break;
                        case "keys":
                        case "fixes":
                        case "typal":
                            $sql = "SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes') . "` WHERE `type` = '$clause' ORDER BY `weight`, `usage` DESC".$limits;
                            break;
                    }
                    $timelimit = 120;
                    $result = $GLOBALS['APIDB']->queryF($sql);
...
if (!function_exists("getExampleNodes")) {
    ...
    {
        $nodes = array();
        $result = $GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('nodes') . "` WHERE LENGTH(`" . $GLOBALS['APIDB']->prefix('nodes') . "`.`node`) > 3 ORDER BY RAND() LIMIT " . mt_rand(3,6));
        while($row = $GLOBALS['APIDB']->fetchArray($result))
            $nodes[] = $row['node'];
            sort($nodes);
            return implode('-', $nodes);
...
if (!function_exists("getExampleFingerprint")) {
    ...
    {
        $nodes = array();
        $result = $GLOBALS['APIDB']->queryF("SELECT DISTINCT `a`.* from `" . $GLOBALS['APIDB']->prefix('fonts') . "` `a` INNER JOIN `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` `b` ON `a`.`id` =  `b`.`font_id` WHERE `a`.`state` = 'online' ORDER BY RAND() LIMIT 1");
        while($row = $GLOBALS['APIDB']->fetchArray($result))
        {
            return $row;
        }
...
if (!function_exists("getExampleFontFiles")) {
    ...
    {
        $fonts = array();
        $result = $GLOBALS['APIDB']->queryF("SELECT * from `" . $GLOBALS['APIDB']->prefix('fonts_archiving') . "` WHERE `font_id` = '$md5'");
        while($row = $GLOBALS['APIDB']->fetchArray($result))
            if (!empty($row['type']))
                $fonts[$row['type']] = $row;
                return $fonts;
...
if (!function_exists("getArchivedZIPFile")) {
    ...
    {
        if (!empty($fontid))
            $GLOBALS['APIDB']->queryF($sql = "UPDATE `" . $GLOBALS['APIDB']->prefix('fonts_files') . "` SET `hits` = `hits` + 1, `accessed` = UNIX_TIMESTAMP() WHERE `font_id` = '" . $fontid . "' AND `filename` = '$zip_file'");
            $data = '';
...
if (!function_exists("getFontsCullList")) {
    ...
    function getFontsCullList($files = array())
    {
        ...
        foreach($ret as $fingerprint => $filevars)
        {
            ...
            if ($count>0)
                $found = true;
                if (strpos(API_URL, 'fonts.snails.email'))
                    $found = false;
                    else {
                        $sql = "SELECT * FROM `" . $GLOBALS['APIDB']->prefix('peers') . "` WHERE `peer-id` NOT LIKE '%s' AND `polinating` = 'Yes'";
                        if ($found == false && $GLOBALS['APIDB']->getRowsNum($results = $GLOBALS['APIDB']->queryF(sprintf($sql, $GLOBALS['APIDB']->escape($GLOBALS['peer-id']))))>=1)
                        {
