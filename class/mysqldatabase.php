...
class DebauchMySQLDatabase extends DebauchDatabase
{
    ...
    public function connect($selectdb = true)
    {
        ...
        if (DB_FONTS_PERS == true) {
            $this->conn = @mysqli_connect(DB_FONTS_HOST, DB_FONTS_USER, DB_FONTS_PASS, DB_FONTS_NAME);
        } else {
            $this->conn = @mysqli_connect(DB_FONTS_HOST, DB_FONTS_USER, DB_FONTS_PASS, DB_FONTS_NAME);
        }
        ...
        if (!$this->conn) {
			...
			if ($selectdb != false) {
				if (!mysqli_select_db($this->conn, DB_FONTS_NAME)) {
					die('Could not select: ' . mysqli_error());
				}
			}
        if (!isset($db_charset_set) && defined('DB_FONTS_CHAR') && DB_FONTS_CHAR) {
            $this->queryF("SET NAMES '" . DB_FONTS_CHAR . "'");
        }
        $db_charset_set = 1;
