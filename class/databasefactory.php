...
class DebauchDatabaseFactory
{
    ...
    static function getDatabaseConnection()
    {
        static $instance;
        if (!isset($instance)) {
            if (file_exists($file = dirname(__FILE__) . '/' . DB_FONTS_TYPE . 'database.php')) {
                require_once $file;

                if (!defined('DB_FONTS_PROXY')) {
                    $class = 'Debauch' . ucfirst(DB_FONTS_TYPE) . 'DatabaseSafe';
                } else {
                    $class = 'Debauch' . ucfirst(DB_FONTS_TYPE) . 'DatabaseProxy';
                }

                /* @var $instance DebauchDatabase */
                $instance = new $class();
                $instance->setPrefix(DB_FONTS_PREF);
                if (!$instance->connect()) {
                    trigger_error('notrace:Unable to connect to database', E_USER_ERROR);
                }
            } else {
                trigger_error('notrace:Failed to load database of type: ' . DB_FONTS_TYPE . ' in file: ' . __FILE__ . ' at line ' . __LINE__, E_USER_WARNING);
            }
        }
        return $instance;
    }
