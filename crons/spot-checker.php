...
while($archive = GLOBALS['FontsDB']->fetchArray($pool))
{
	...
	switch($font['medium'])
	{
		...
		case 'FONT_RESOURCES_PEER':
			$sessions = unserialize(file_get_contents(FONT_RESOURCES_CACHE . DIRECTORY_SEPARATOR . "file-store-sessions.serial"));
			if (!file_exists(constant(FONT_RESOURCES_CACHE) . $archive['path'] . DIRECTORY_SEPARATOR . $archive['filename']) && !isset($sessions[md5($archive['path'] . DIRECTORY_SEPARATOR . $archive['filename'])]))
			{
				...
				if ($GLOBALS['FontsDB']->getRowsNum($results = $GLOBALS['FontsDB']->queryF(sprintf($sql, $GLOBALS['FontsDB']->escape($font['peer_id']))))==1)
				{
					...
					mkdir(constant("FONT_RESOURCES_CACHE") . $archive['path'], 0777, true);
					writeRawFile(constant("FONT_RESOURCES_CACHE") . $archive['path'] . DIRECTORY_SEPARATOR . $archive['filename'], getURIData(sprintf($peer['api-uri'].$peer['api-uri-zip'], $archive['font_id'])));
					$sessions[md5($archive['path'] . DIRECTORY_SEPARATOR . $archive['filename'])] = array("opened" => microtime(true), "dropped" => microtime(true) + mt_rand(3600 * 0.785, 3600 * 1.896), "resource" => $archive['path'] . DIRECTORY_SEPARATOR . $archive['filename']);
				}
			} else {
				if ($sessions[md5($archive['path'] . DIRECTORY_SEPARATOR . $archive['filename'])]['dropped'] < microtime(true) + ($next = mt_rand(1800*.3236, 2560*.5436)))
			...
			writeRawFile(FONT_RESOURCES_CACHE . DIRECTORY_SEPARATOR . "file-store-sessions.serial", serialize($sessions));
			$json = json_decode(getArchivedZIPFile($file = FONT_RESOURCES_CACHE . $archive['path'] . DIRECTORY_SEPARATOR . $archive['filename'], 'font-resource.json'), true);
			break;
	}
	...
	if ($updated==true)	
	{
		...
		if (file_exists($packfile))
		{
			...
			if (!$GLOBALS['FontsDB']->queryF($sql = "UPDATE `fonts` SET `nodes` = '$nodecount', `medium` = 'FONT_RESOURCES_RESOURCE' WHERE `id` = '$fingerprint'"))
				die("Failed SQL: $sql;\n");
