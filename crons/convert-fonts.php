...
$result = $GLOBALS['FontsDB']->queryF("SELECT * from `uploads` WHERE `uploaded` > '0' AND `converted` <= '0' ORDER BY RAND() LIMIT 27");
			...
			foreach(array("ttf", "otf", "woff") as $type)
			{
				if (file_exists($grader[$type]))
				{
					...
					$data = array(	"CSS"=>array($fontage->getFontFullName() => $fontage->getFontFullName() . ".css"),'FontType' => $fontage->getFontType(), 'FontCopyright' => $fontage->getFontCopyright(), "getFontName" => $fontage->getFontName(),
									...
									"Files" => $files,'Table' => $fontage->getTable(), "UnicodeCharMap" => $fontage->getUnicodeCharMap(),
									"Reserves" => $reserves);
					...
					$GLOBALS['FontsDB']->queryF("UPDATE `uploads` SET `currently_path` = '".$currently."', `converted` = '".time()."', `font_id` = '$fingerprint', `datastore` = '" . json_encode(array_merge(json_decode($row['datastore'], true), array("files" => $files, "FontName" => $fontage->getFontFullName(), "FontFamily" => $fontage->getFontSubfamily()))) . "' WHERE `id` = " . $row['id']);
					unset($fontage);
					unset($grader);
					continue;
				}
