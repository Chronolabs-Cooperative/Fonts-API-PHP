...
CREATE TABLE `fonts_files` (
  ...
  `accessings` int(42) DEFAULT NULL,
  `updates` int(20) NOT NULL DEFAULT '0',
  `caching` int(20) DEFAULT NULL,
  `sourcings` int(42) DEFAULT NULL,
  `created` int(13) NOT NULL DEFAULT '0',
  `accessed` int(13) NOT NULL DEFAULT '0',
  `updated` int(13) NOT NULL DEFAULT '0',
  `cached` int(13) DEFAULT NULL,
  `sourced` int(13) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `SEARCH` (`font_id`(14),`archive_id`,`type`,`extension`,`filename`(12),`path`,`id`),
  KEY `CHRONOLOGISTIC` (`updated`,`accessed`,`created`,`bytes`,`path`,`filename`,`extension`,`font_id`,`id`,`archive_id`,`cached`,`sourced`)
